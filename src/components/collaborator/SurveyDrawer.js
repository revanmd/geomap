...
export default function SurveyDrawer({
  ...
  return (
    ...
    >
      ...
      <h2 className="text-sm mt-4 px-4 font-medium">
        <span className="font-semibold text-red-600">*</span> Satuan Tanah
        <span className="font-light text-gray-500"> (Otomatis dari peta)</span>
      </h2>
      <div className="mx-4 mt-2">
        <Input
          className="input-custom bg-gray-100"
          placeholder="Klik pada peta untuk mendeteksi satuan tanah"
          value={satuanTnh}
          disabled={true}
          readOnly
        />
      ...
      <div className="px-4 mt-2">
        <div
          className={`image-commodity-container-empty border rounded ${isSubmitting
      ...
      <div className="mx-4">
        <button
          className={`mt-4 font-semibold text-white text-center text-sm p-2 rounded w-full ${(surveyCommodity !== "" && satuanTnh !== "" && (capturedImage || uploadedImage) && !isSubmitting) ? 'bg-blue' : 'bg-blue-200'
            }`}
          disabled={!(surveyCommodity !== "" && satuanTnh !== "" && (capturedImage || uploadedImage)) || isSubmitting}
          onClick={onFinish}
        >
          {isSubmitting ? "Menyimpan..." : mode === "create" ? "Simpan Komoditas" : "Simpan Perubahan"}
        </button>
        <button
          className={`mb-4 mt-2 font-semibold text-center text-sm p-2 rounded border w-full ${isSubmitting
