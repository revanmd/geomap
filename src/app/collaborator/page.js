...
function CollaboratorContent() {
  ...
  const surveyForm = useSurveyForm({
    ...
    onError: (error) => {
      showMessage(
        error.message || "Gagal menambahkan komoditas",
        <CancleIcon />,
      );
    },
  });
  ...
  const callbackPressMap = (params) => {
    resetSurvey();
    setSurveyStep(1);
    // Auto-fill Satuan_Tnh from vector tile layer
    if (params?.satuan_tnh) {
      surveyForm.setSatuanTnh(params.satuan_tnh);
    }
  };
  ...
  const fetchMarkerDetail = async (markerId) => {
    ...
    try {
      ...
      showMessage(
        error.message || "Gagal mengambil detail marker",
        <CancleIcon />,
      );
  ...
  const handleFinishEdit = async () => {
    ...
    try {
      ...
      const updatedData = await surveyForm.updateMarker(
        markerDetail.id,
        location,
      );
      ...
      if (
        mapFunctions &&
        updatedData?.location?.lat &&
        updatedData?.location?.lon
      ) {
  ...
  const finishSurvey = async () => {
    ...
    try {
      ...
      if (!markerLocation) {
        throw new Error(
          "Lokasi marker tidak ditemukan. Silakan pilih lokasi pada peta terlebih dahulu.",
        );
      }

      // Check radius: block if same commodity already exists within radius
  ...
  return (
    <main className="h-[100dvh] w-screen relative">
      ...
      >
        ...
        {event == "survey" && surveyStep == 0 && (
          <AnimatePresence>
            ...
            >
              <div className="bg-white w-screen fixed bottom-0">
                <div className="m-3">
                  <div className="bg-blue-100 flex p-3 items-center">
                    ...
                    <div className="flex-auto text-xs font-semibold">
                      Tekan dan tahan selama 2 detik pada area di dalam radius
                      biru untuk menentukan lokasi komoditas.
                    </div>
                  </div>
                </div>
              </div>
      ...
      {event == "survey" && (
        ...
        >
          <div
            className="bg-white ml-5 p-3 rounded-full text-blue shadow-lg"
